...
public class Command_cex_xp {
	...
	public static Boolean run(CommandSender sender, String alias, String[] args){
		...
		if (function.equalsIgnoreCase("add")){
			...
			if (amountint - xpLimit > 0){
				if (!spaceToAdd) { LogHelper.showInfo("xpCouldNotAddAll", sender, ChatColor.RED); }
				if (sender != target) { LogHelper.showInfo("xpAdded#####[" + (spaceToAdd ? amountint : (amountint - xpLimit < 0 ? 0 : amountint - xpLimit)) + " #####xpExperience#####[ #####xpTo#####[" + target.getName(), sender, ChatColor.GREEN); }
				LogHelper.showInfo((sender != target ? "#####[" + sender.getName() + " #####xpAddedGave1#####[" : "xpAddedGave2#####[") + (spaceToAdd ? amountint : amountint - xpLimit) + " #####xpExperience", target, ChatColor.AQUA);
			} else {
